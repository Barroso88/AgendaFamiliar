                ...
                <div class="flex-1" style="${e.isTask ? 'color: white !important;' : 'color: #022c22 !important;'}">
                    ...
                    <div class="flex items-center gap-2 mt-2">
                        ...
                        <div class="flex -space-x-1">
                            ${e.members.map(m => { const memb = getMember(m); return memb ? `<div class="w-5 h-5 rounded-full ${getMemberColor(m)} flex items-center justify-center text-[10px]" title="${memb.name}">${memb.avatar}</div>` : ''; }).join('')}
                        </div>
                    </div>
                </div>
...
function openEventModal(eventId = null, defaultDate = null) {
    ...
    <div class="p-6">
        ...
        <form onsubmit="saveEvent(event, ${eventId ? `'${eventId}'` : 'null'})" class="space-y-4">
            <div>
                ...
                <div class="bg-white dark:bg-gray-800 rounded-xl p-4 border transition-all relative overflow-hidden ${borderClass} ${opacityClass} flex flex-col md:flex-row gap-4 items-start md:items-center group">
                    ...
                    <div class="flex items-center justify-between md:justify-end gap-4 mt-2 md:mt-0 w-full md:w-auto">
                        <div class="flex -space-x-2">
                            ${e.members.map(m => { const memb = getMember(m); return memb ? `<div class="w-8 h-8 rounded-full ${getMemberColor(m)} flex items-center justify-center text-xs border-2 border-white dark:border-gray-800 shadow-sm" title="${memb.name}">${memb.avatar}</div>` : ''; }).join('')}
                        </div>
